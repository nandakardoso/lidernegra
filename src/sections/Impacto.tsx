import { indicadoresImpacto } from "../data/impacto";
import { TextLink } from "../components/TextLink";

export function Impacto({ comLink = false }: { comLink?: boolean }) {
  return (
    <section id="impacto" className="scroll-mt-20 bg-wine-950 py-24 text-cream-50">
      <div className="mx-auto max-w-6xl px-6 md:px-10">
        <p className="font-sans text-xs font-semibold uppercase tracking-[0.2em] text-gold-400">Impacto</p>
        <h2 className="mt-4 font-display text-3xl italic md:text-4xl">O que a rede já construiu</h2>

        <dl className="mt-14 grid gap-10 sm:grid-cols-2 lg:grid-cols-5">
          {indicadoresImpacto.map((item) => (
            <div key={item.label} className="flex flex-col border-t border-gold-500/40 pt-5">
              <dt className="order-2 mt-3 font-sans text-sm leading-snug text-cream-100/85">{item.label}</dt>
              <dd className="order-1 font-display text-4xl italic text-gold-400">{item.valor}</dd>
            </div>
          ))}
        </dl>

        <p className="mt-10 max-w-2xl font-sans text-sm text-cream-100/70">
          Dados informados pelo LÍDERNEGRA. Os percentuais de 86% e 73% se referem às participantes da
          primeira turma.
        </p>

        {comLink && (
          <div className="mt-8">
            <TextLink to="/sobre#impacto" tone="onDark">
              Ver mais sobre o impacto
            </TextLink>
          </div>
        )}
      </div>
    </section>
  );
}
